...
def delete_item_on_table():
    try:
        response = get_dynamodb_resource().Table("Movies_1").delete_item(
            Key={
                'year': 2015,
                'title': "The Big New Movie"
            },
...
def insert_sample_data():
    table = get_dynamodb_resource().Table("Movies_1")

    with open(r'C:\Users\Santhosh Raj\Downloads\bulk_load_dynamo\movies_sample.json') as json_file:
...
def query_movies_released_in_1985():
    response = get_dynamodb_resource().Table('Movies_1').query(
...
def scan_whole_table_for_items():
    ...
    response = get_dynamodb_resource().Table("Movies_1").scan(
