...
def dynamodb_to_pandas_dataframe(table_name):
    ...
    df['site'] = df['site'].str.replace('.0', '', regex=False)
    
    #df['site'] = df['site'].astype(int)
    #df['Retention'] = df['Retention'].astype('string')
    #df['AE'] = df['AE'].astype('string')
    #df['SAE'] = df['SAE'].astype('string')
...
if __name__ == "__main__":
    ...
    table_name = "dev-ofr-datahub-cms-clinical-study-standard-view"
    bucket = "dev-data-lake-cms-conform-eu-west-1-239126490696"
    prefix = "dynamodb_data/studystandardviewnorm"
    database = "dev_data_lake_cms_conform_glue_database"
    table = "study_standard_normalized"
